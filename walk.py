...
class WALK:
	def __init__(self, master):
		...
		self.bg_img = ImageTk.PhotoImage(file = os.path.join('Assets', os.path.join('Walk', 'background.png')))
		self.bg_render = self.canvas.create_image((0, 0), image = self.bg_img, anchor = N + W)
		
		self.label_msg_render = self.canvas.create_text((520, 50), text = 'IVAST Bot - Walking', font = ('Transformers Movie', 40), fill = '#CCCCFF')
